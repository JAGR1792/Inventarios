...
def _file_url(path: str | None) -> str | None:
    ...
    if not p.exists():
        return None
    url = QtCore.QUrl.fromLocalFile(str(p)).toString()
    try:
        # Bust WebEngine cache on updates; helps instant image refresh.
...
class WebBackend(QtCore.QObject):
    def __init__(self, session_factory, settings: Settings, parent: QtCore.QObject | None = None):
    ...
    @QtCore.Slot(str, result="QVariant")
    def getDailyTotals(self, day_iso: str):
        with session_scope(self._session_factory) as session:
            sales = SalesRepo(session)
            t = sales.totals_for_day(day_iso)
        return {
            "ok": True,
    ...
    def getCashPanel(self, day_iso: str):
        ...
        with session_scope(self._session_factory) as session:
            ...
            # If opening cash is not manual, keep it aligned to the suggested carry.
            try:
            ...
            expected_cash_end = (
                Decimal(str(day_row.opening_cash or 0))
                + Decimal(str(t["cash_total"] or 0))
                - withdrawals_total
    ...
    @QtCore.Slot(str, "QVariant", str, result="QVariant")
    ...
    @QtCore.Slot(str, "QVariant", "QVariant", str, result="QVariant")
    ...
    @QtCore.Slot(str, "QVariant", str, result="QVariant")
    def createCashClose(self, day_iso: str, cash_counted, notes: str = ""):
        day = (day_iso or "").strip()
        if not day:
            return {"ok": False, "error": "Día inválido"}
        ...
        try:
            ...
            return {"ok": False, "error": "Efectivo contado inválido"}
        ...
        with session_scope(self._session_factory) as session:
            ...
            diff: Decimal | None = None
            if cash_counted_d is not None:
                diff = (cash_counted_d - t["cash_total"]).quantize(Decimal("0.01"))
    ...
    def resetDatabase(self, confirm_text: str):
        ...
        # IMPORTANT: Never delete product images (files or DB mappings).
        # We reset operational data: sales + cash movements, and set stock to 0.
        try:
            with session_scope(self._session_factory) as session:
                # Sales
                ...
                # Keep catalog + categories + image mappings; only reset stock.
    ...
    def openImagesFolder(self):
        try:
            img_dir = (self._settings.INSTANCE_DIR / str(self._settings.IMAGES_DIR)).resolve()
            img_dir.mkdir(parents=True, exist_ok=True)
            ok = QtGui.QDesktopServices.openUrl(QtCore.QUrl.fromLocalFile(str(img_dir)))
